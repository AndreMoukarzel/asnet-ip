(define (domain blocksworld)
  ...
  (:action pick-tower
    ...
    :effect
      (imprecise (0.0 .1) (and (holding ?b2) (clear ?b3) (not (emptyhand)) (not (on ?b2 ?b3))))
  )
  (:action put-tower-on-block
    :parameters (?b1 ?b2 ?b3 - block)
    :precondition (and (holding ?b2) (on ?b1 ?b2) (clear ?b3) (not (equal ?b1 ?b2)) (not (equal ?b2 ?b3)) (not (equal ?b1 ?b3)))
    :effect (imprecise (0.0 .1) (and (on ?b2 ?b3) (emptyhand) (not (holding ?b2)) (not (clear ?b3)))
                       (0.9 1.) (and (on-table ?b2) (emptyhand) (not (holding ?b2))))
  )
